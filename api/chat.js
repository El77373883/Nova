...
async function fetchKnowledgeFromR2() {
  ...
  const canonicalUri     = '/' + bucket + '/knowledge.json';
...
function findBestMatch(question, kvData) {
  ...
  for (const [key, value] of Object.entries(kvData)) {
    ...
    const matches  = questionWords.filter(word => keyWords.some(kw => kw.includes(word) || word.includes(kw))).length;
    const score    = matches / Math.max(questionWords.length, 1);
    if (score > bestScore) { bestScore = score; bestMatch = value; }
  }
  if (bestScore < 0.3 || !bestMatch) return 'Lo siento, no tengo informacion sobre ese tema.';
  return bestMatch;
}
